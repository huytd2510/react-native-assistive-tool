...
const KeyValueTable = () => {
  ...
  const loadMoreData = async () => {
    ...
    if (searchQuery.trim() !== '') {
      filteredKeys = keys.filter(key => key.toLowerCase().includes(searchQuery.toLowerCase()));
    }
  ...
  const renderItem = ({
    ...
    numberOfLines: 10,
    ellipsizeMode: 'tail'
  }, item.value.substring(0, 100), item.value.length > 100 ? '  ...' : '')), /*#__PURE__*/_react.default.createElement(_reactNative.View, {
